...
def handle_request(url):
    ...
    if os.path.exists(cache_path):
        with open(cache_path, 'rb') as f:
            content = f.read()
        # Update the access frequency and last access time in the lookup table
...
def update_lookup_table(url, accessed=False):
    ...
    If 'accessed' is True, update the access frequency and last access time.
    """
    global lookup_df
    cache_path = os.path.join(cache_directory, url)
...
def manage_cache_eviction():
    """
    Evict the least frequently used items from the cache if it exceeds the maximum size.
    """
    global lookup_df

    total_cache_size = get_total_cache_size()
    if total_cache_size > max_cache_size:
        # Sort the lookup table by Access_Frequency (ascending) to find the least frequently used items
        lookup_df.sort_values(by='Access_Frequency', inplace=True)
